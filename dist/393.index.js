...
async function getAuthHeaders(registry, repository) {
    try {
        ...
        const authenticateHeader = new www_authenticate.parsers.WWW_Authenticate(apiCheckResponse.headers['www-authenticate']);
        ...
        logger/* default.error */.Z.error(source.red('auth error'), err.message);
        throw new Error('Failed to obtain docker registry token');
    }
}
    ...
    try {
        ...
        switch (resp.headers['content-type']) {
            ...
            case DockerContentType.ManifestV1Signed:
                ...
                logger/* default.warn */.Z.warn(source.yellow('Wrong response'), `Wrong response: ${resp.headers['content-type']}`);
                return '<error>';
            default:
        ...
        logger/* default.error */.Z.error(source.red('request error'), e.message);
...
async function publish({ image, imagePrefix, tag, dryRun, }) {
    ...
    logger/* default.info */.Z.info(source.blue('Processing image:'), source.yellow(fullName));
    ...
    if (oldId === newId) {
        (0,logger/* default */.Z)('Image uptodate, no push nessessary:', source.yellow(oldId));
        return;
    }
    (0,logger/* default */.Z)('Publish new image', `${oldId} => ${newId}`);
    if (dryRun) {
        logger/* default.warn */.Z.warn(source.yellow('[DRY_RUN]'), source.blue('Would push:'), fullName);
    }
    else {
        await (0,common/* docker */.e$)('push', fullName);
    }
    logger/* default.info */.Z.info(source.blue('Processing image finished:', newId));
}
...
/* harmony default export */ const ubuntu = ((/* unused pure expression or super */ null && (api)));
